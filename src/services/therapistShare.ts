...
export async function exportReportAsText(report: TherapistReport): Promise<string> {
  ...
  text += '                           DISCLAIMER                           \n';
  text += '═══════════════════════════════════════════════════════════════\n\n';
  text += `This report is generated by ${APP_CONFIG.name} based on user self-reported data.\n`;
  ...
  text += `Generated by ${APP_CONFIG.name} — The Human Cockpit\n`;
...
export async function shareReport(report: TherapistReport): Promise<void> {
  ...
  const fileName = `${APP_CONFIG.name.replace(/\s+/g, '_')}_Report_${format(new Date(), 'yyyy-MM-dd')}.txt`;
  ...
  if (await Sharing.isAvailableAsync()) {
    await Sharing.shareAsync(filePath, {
      mimeType: 'text/plain',
      dialogTitle: `Share ${APP_CONFIG.name} Report`,
    });
  }
}
