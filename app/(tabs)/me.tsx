    ...
    >
      ...
      <View style={styles.section}>
        ...
        <View style={styles.settingsCard}>
          ...
          <Pressable style={styles.settingRow} onPress={() => router.push('/(modals)/history')}>
            <Text style={styles.settingLabel}>Conversation History</Text>
            ...
            <Pressable style={styles.sensitiveModalCard} onPress={() => {}}>
              ...
              <ScrollView style={styles.sensitiveChipScroll}>
                {SENSITIVE_TOPICS_TOGGLES.map((topic) => {
                  ...
                  const isEnabled = userSensitiveTopics.includes(topic.id);
                  const toggleSensitiveTopic = (id: string) => {
                    if (userSensitiveTopics.includes(id))
                      setSensitiveTopics(userSensitiveTopics.filter((t) => t !== id));
                    else setSensitiveTopics([...userSensitiveTopics, id]);
                  };
                  return (
                    <Pressable
                      key={topic.id}
                      onPress={() => toggleSensitiveTopic(topic.id)}
                      style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.06)' }}
                    >
                      <Text style={{ color: COLORS.text, fontSize: 15 }}>{topic.label}</Text>
                      <View style={{ width: 44, height: 24, borderRadius: 12, backgroundColor: isEnabled ? COLORS.accent : 'rgba(255,255,255,0.1)', justifyContent: 'center', paddingHorizontal: 2 }}>
                        <View style={{ width: 20, height: 20, borderRadius: 10, backgroundColor: '#fff', alignSelf: isEnabled ? 'flex-end' : 'flex-start' }} />
                      </View>
                    </Pressable>
                  );
                })}
