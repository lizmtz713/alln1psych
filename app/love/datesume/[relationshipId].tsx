/**
 * Datésumé — View/edit single relationship experience.
 */
...
export default function RelationshipIdScreen() {
  const insets = useSafeAreaInsets();
  const router = useRouter();
  const { relationshipId } = useLocalSearchParams<{ relationshipId: string }>();
  const { datesume, init, updateRelationship, removeRelationship } = useDatesumeStore();
  const rel = datesume?.relationships.find((r) => r.id === relationshipId);
  ...
  useEffect(() => {
    if (rel) {
      setTitle(rel.title);
      setType(rel.type);
      setPartnerName(rel.partnerName ?? '');
      setIsAnonymous(rel.isAnonymous);
      setStartYear(rel.startYear);
      setIsOngoing(rel.isOngoing);
      setEndYear(rel.endYear);
      setWhatIBrought(rel.whatIBrought ?? []);
      setWins(rel.wins ?? []);
      setHowItEnded(rel.howItEnded ?? '');
      setWhoEnded(rel.whoEnded);
      setCurrentStatus(rel.currentStatus ?? '');
      setLessonsLearned(rel.lessonsLearned ?? []);
    }
  }, [rel?.id]);

  if (!relationshipId || !rel) {
    return (
      <View style={styles.center}>
        <Text style={styles.muted}>Experience not found.</Text>
  ...
  const handleDelete = () => {
    Alert.alert('Delete experience', 'Remove this relationship from your Datésumé?', [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: () => { removeRelationship(relationshipId); router.back(); } },
    ]);
  };
...
const styles = StyleSheet.create({
  ...
  backBtn: { marginTop: 16, paddingVertical: 12, paddingHorizontal: 24 },
  ...
  saveBtn: { backgroundColor: ACCENT, paddingVertical: 16, borderRadius: BORDER_RADIUS.button, alignItems: 'center', marginBottom: 12 },
