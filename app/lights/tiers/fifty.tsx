...
export default function YourFiftyScreen() {
  ...
  const fifty = lights.filter((l) => l.tier === 'fifty');

  return (
    <ScrollView style={[styles.container, { paddingBottom: insets.bottom + 24 }]} contentContainerStyle={styles.content}>
      <Text style={styles.title}>Your 50 — Friends</Text>
      <Text style={styles.count}>{fifty.length} of 50 slots filled</Text>
      {fifty.length === 0 ? (
        <Text style={styles.empty}>No one in Your 50 yet.</Text>
      ) : (
        fifty.map((l) => (
