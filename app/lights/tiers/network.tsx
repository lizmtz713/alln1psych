...
export default function YourNetworkScreen() {
  ...
  const network = lights.filter((l) => l.tier === 'network');

  return (
    <ScrollView style={[styles.container, { paddingBottom: insets.bottom + 24 }]} contentContainerStyle={styles.content}>
      <Text style={styles.title}>Your 150 — Acquaintances</Text>
      <Text style={styles.count}>{network.length}/150 in your network</Text>
      {network.length === 0 ? (
        <Text style={styles.empty}>No one in Your 150 yet.</Text>
      ) : (
        network.map((l) => (
...
const styles = StyleSheet.create({
  ...
  count: { fontSize: 14, color: COLORS.textMuted, marginBottom: 20 },
